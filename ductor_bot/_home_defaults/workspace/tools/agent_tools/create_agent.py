...
def _agents_path() -> Path:
    ...
    Sub-agents have DUCTOR_HOME = ~/.jarvis/agents/<name>/, so we navigate
    up to the main home. Main agent's DUCTOR_HOME points directly to ~/.jarvis/.
    """
    import os

    home = Path(os.environ.get("DUCTOR_HOME", str(Path.home() / ".jarvis")))
...
def _main_home() -> Path:
    ...
    home = Path(os.environ.get("DUCTOR_HOME", str(Path.home() / ".jarvis")))
    if home.name != ".jarvis" and (home.parent.parent / "config").is_dir():
